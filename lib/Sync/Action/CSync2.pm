...
sub BUILD {
    ...
    if ( !$self->config->{'prod_dir'} or !$self->config->{'csync_dir'} or
        ...
        foreach my $i ( 1 .. 100 ) {
            ...
            if ( $err or scalar( @{ $self->files } ) ) {
                ...
                while ($diff_ts < $self->_retry_interval) {
                    ...
                    $self->log->debug("process_files(): delaying next run by " .
                        "${sleep}s");
                    sleep ( $sleep );
